...
#>
    ...
    Process
    {
        ...
        {
            Write-Log -Message "Failed to get OS AddressWidth. `n$(Resolve-Error)" -Severity 3 -Source ${CmdletName}
        }
                
        if ($Arch -eq '64')
        {
            Write-Log -Message "64-Bit OS Architecture" -Severity 1 -Source ${CmdletName} 
            Return '64-Bit'
        } 
        elseif ($Arch -eq '32')
        {
            Write-Log -Message "32-Bit OS Architecture" -Severity 1 -Source ${CmdletName}
            Return '32-Bit'
        }
        else 
        {
            Write-Log -Message "OS Architecture not found!" -Severity 3 -Source ${CmdletName}
            Throw "OS Architecture not found!"
        }
    }
